...
def parse_category(src):
    ...
    products = soup.select('.name-block')
...
def parse_page(products_info, prices_info, src):
    ...
    for idx, item in enumerate(products_info):
        title = item.getText()
        price_float = prices_info[idx].getText().replace(' \nР', '').strip()
        ready_to_convert_price = price_float.replace(' ', '')
        ready_to_convert_price = ready_to_convert_price[0:-2]
...
def detect_site(src):
    site_index = src.index('.ru')
    site_address = src[12:site_index+3]
    return site_address
